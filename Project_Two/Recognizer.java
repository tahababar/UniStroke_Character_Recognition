...
public class Recognizer {
    ...
    {   double scaleFactor;
        ...
        // assign max the larger of the maximum x or maximum y in the stroke
     ...
     *            the score
     */
    public double computerScore(int IndexDigitToCompare)
    {   double Score =0;
        for (int i=0; i<nextFree;i++) {
    ...
    public int findMatch()
    {
        ...
        double minScore = computerScore(0);
        int match=0;

        for (int i=1; i<=9; i++)
        { 
            if(minScore>computerScore(i))
               {match = i;
                 minScore = computerScore(i); 
               } 
     ...
     */
    public int findMinX()
    {   int minValue = userStroke[0].x;
        for (int i = 0; i < nextFree ; i++)
        { 
            if (userStroke[i].x < minValue)
               {minValue = userStroke[i].x;}
     ...
     */
    public int findMinY()
    {   int minValue = userStroke[0].y;
        for (int i = 0; i < nextFree ; i++)
        { 
            if (userStroke[i].y < minValue)
               {minValue = userStroke[i].y;}
     ...
     */
    public int findMaxX()
    {   int maxValue = userStroke[0].x;
        for (int i = 0; i < nextFree ; i++)
        { 
            if (userStroke[i].x > maxValue)
               {maxValue = userStroke[i].x;}
     ...
     */
    public int findMaxY()
    {   int maxValue = userStroke[0].y;
        for (int i = 0; i < nextFree ; i++)
        { 
            if (userStroke[i].y > maxValue)
               {maxValue = userStroke[i].y;}
